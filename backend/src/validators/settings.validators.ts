...
export const validateNotifications = (req: Request, res: Response, next: NextFunction): void => {
...
export const validatePlatformSettings = (req: Request, res: Response, next: NextFunction): void => {
  const { lmsName, supportEmail, sessionTimeout } = req.body;

  if (lmsName !== undefined && (typeof lmsName !== 'string' || lmsName.trim().length === 0)) {
