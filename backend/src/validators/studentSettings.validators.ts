...
export const validateStudentProfile = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (gender !== undefined && gender !== '' && !['Male', 'Female', 'Other', 'Prefer not to say'].includes(gender)) {
    res.status(400).json({ status: 'error', message: 'Please select a valid gender option.' });
    return;
  }
...
export const validateStudentNotifications = (req: Request, res: Response, next: NextFunction): void => {
...
export const validateStudentPreferences = (req: Request, res: Response, next: NextFunction): void => {
