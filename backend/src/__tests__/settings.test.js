...
describe("Admin Settings API Endpoints", () => {
  ...
  describe("Profile Endpoints", () => {
    it("should return profile data for authenticated admin", async () => {
      const res = await request(app).get("/api/admin/settings/profile").set("Authorization", `Bearer ${adminToken}`);
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("success");
    });
    it("should validate email format on profile update", async () => {
      const res = await request(app).patch("/api/admin/settings/profile").set("Authorization", `Bearer ${adminToken}`).send({ email: "invalid-email-format" });
  ...
  describe("Password Change Validation", () => {
    it("should reject weak password change requests", async () => {
      const res = await request(app).post("/api/admin/settings/change-password").set("Authorization", `Bearer ${adminToken}`).send({
  ...
  describe("Notifications & Preferences", () => {
    it("should retrieve notification preferences", async () => {
      const res = await request(app).get("/api/admin/settings/notifications").set("Authorization", `Bearer ${adminToken}`);
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("success");
    });
    it("should validate theme value on preferences update", async () => {
      const res = await request(app).patch("/api/admin/settings/preferences").set("Authorization", `Bearer ${adminToken}`).send({ theme: "InvalidTheme" });
      expect(res.status).toBe(400);
  ...
  describe("Platform Settings Validation", () => {
    it("should validate session timeout limits", async () => {
      const res = await request(app).patch("/api/admin/settings/platform").set("Authorization", `Bearer ${adminToken}`).send({ sessionTimeout: 2 });
      expect(res.status).toBe(400);
