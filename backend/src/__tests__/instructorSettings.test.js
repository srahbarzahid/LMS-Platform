...
const generateToken = (role = "INSTRUCTOR") => {
...
describe("Instructor Settings API Endpoints", () => {
  ...
  describe("Authentication & Role Protection", () => {
    ...
    it("should allow instructor access to instructor profile", async () => {
      const res = await request(app).get("/api/instructor/settings/profile").set("Authorization", `Bearer ${instructorToken}`);
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("success");
    });
    it("integration test: confirming an instructor CANNOT access admin platform settings (403 Forbidden)", async () => {
      const res = await request(app).get("/api/admin/settings/platform").set("Authorization", `Bearer ${instructorToken}`);
  ...
  describe("Instructor Profile Endpoints & Designation/Bio Validation", () => {
    ...
    it("should reject profile update with invalid email format", async () => {
      const res = await request(app).patch("/api/instructor/settings/profile").set("Authorization", `Bearer ${instructorToken}`).send({ email: "bad-email-format" });
      expect(res.status).toBe(400);
      expect(res.body.message).toContain("valid email address");
    });
    it("should successfully update designation and bio", async () => {
      const res = await request(app).patch("/api/instructor/settings/profile").set("Authorization", `Bearer ${instructorToken}`).send({
        name: "Sarah Jenkins",
  ...
  describe("Password Change Validation", () => {
    it("should reject weak password change requests", async () => {
      const res = await request(app).post("/api/instructor/settings/change-password").set("Authorization", `Bearer ${instructorToken}`).send({
  ...
  describe("Notification JSON Preferences", () => {
    it("should retrieve instructor notification preferences", async () => {
      const res = await request(app).get("/api/instructor/settings/notifications").set("Authorization", `Bearer ${instructorToken}`);
      expect(res.status).toBe(200);
    ...
    it("should update JSON notification preferences for student activity", async () => {
      ...
      const res = await request(app).patch("/api/instructor/settings/notifications").set("Authorization", `Bearer ${instructorToken}`).send({ notificationPreferences: payload });
      expect(res.status).toBe(200);
      expect(res.body.status).toBe("success");
    });
  });
  describe("UI Preferences", () => {
    it("should validate theme value on preferences update", async () => {
      const res = await request(app).patch("/api/instructor/settings/preferences").set("Authorization", `Bearer ${instructorToken}`).send({ theme: "UnsupportedTheme" });
      expect(res.status).toBe(400);
  ...
  describe("Account Deactivation (Soft Delete)", () => {
    it("should require password confirmation to deactivate account", async () => {
      const res = await request(app).post("/api/instructor/settings/deactivate").set("Authorization", `Bearer ${instructorToken}`).send({});
      expect(res.status).toBe(400);
