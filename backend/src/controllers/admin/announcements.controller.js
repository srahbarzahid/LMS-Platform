let mockAnnouncements = [
  {
    ...
    publishDate: /* @__PURE__ */ new Date("2026-07-15T02:00:00Z"),
    expiryDate: /* @__PURE__ */ new Date("2026-07-15T04:00:00Z"),
    sendDashboardNotification: true,
    sendEmailNotification: false,
    createdAt: /* @__PURE__ */ new Date("2026-07-01T10:00:00Z")
  },
  {
    ...
    publishDate: /* @__PURE__ */ new Date("2026-07-01T08:00:00Z"),
    expiryDate: /* @__PURE__ */ new Date("2026-12-31T23:59:00Z"),
    sendDashboardNotification: true,
    sendEmailNotification: true,
    createdAt: /* @__PURE__ */ new Date("2026-06-25T14:30:00Z")
  },
  {
    ...
    status: "Published",
    publishOption: "Publish Now",
    publishDate: /* @__PURE__ */ new Date("2026-07-05T09:00:00Z"),
    expiryDate: null,
    sendDashboardNotification: true,
    sendEmailNotification: true,
    createdAt: /* @__PURE__ */ new Date("2026-07-04T16:20:00Z")
  },
  {
    ...
    sendEmailNotification: false,
    createdAt: /* @__PURE__ */ new Date("2026-07-08T11:15:00Z")
  }
];
const adminAnnouncementsController = {
  ...
  getAllAnnouncements: async (req, res) => {
    try {
      res.status(200).json({ success: true, data: mockAnnouncements });
    } catch (error) {
      res.status(500).json({ success: false, message: "Server error" });
    }
  },
  ...
  getAnnouncementById: async (req, res) => {
    try {
      ...
      const announcement = mockAnnouncements.find((a) => a.announcementId === announcementId);
      if (!announcement) {
    ...
    try {
      ...
      if (newAnnouncement.publishOption === "Save Draft") {
        ...
        if (!newAnnouncement.publishDate) {
          newAnnouncement.publishDate = /* @__PURE__ */ new Date();
        }
      }
  ...
  updateAnnouncement: async (req, res) => {
    try {
      const { announcementId } = req.params;
      const index = mockAnnouncements.findIndex((a) => a.announcementId === announcementId);
      if (index === -1) {
        return res.status(404).json({ success: false, message: "Announcement not found" });
      }
      ...
      mockAnnouncements[index] = { ...mockAnnouncements[index], ...updatedData };
      res.status(200).json({ success: true, data: mockAnnouncements[index] });
    } catch (error) {
      res.status(500).json({ success: false, message: "Server error" });
    }
  },
  ...
  deleteAnnouncement: async (req, res) => {
    try {
      const { announcementId } = req.params;
      mockAnnouncements = mockAnnouncements.filter((a) => a.announcementId !== announcementId);
  ...
  togglePublishStatus: async (req, res) => {
    try {
      ...
      const currentStatus = mockAnnouncements[index].status;
      ...
      mockAnnouncements[index] = {
        ...
        status: newStatus,
        publishOption,
        publishDate: newStatus === "Published" ? /* @__PURE__ */ new Date() : mockAnnouncements[index].publishDate
      };
      res.status(200).json({
        success: true,
