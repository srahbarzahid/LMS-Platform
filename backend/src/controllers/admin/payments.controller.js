let mockPayments = Array.from({ length: 45 }).map((_, i) => {
  ...
  const amount = Math.floor(Math.random() * 5e3) + 500;
  return {
    id: `PAY${1e3 + i}`,
    studentName: `Student ${i + 1}`,
    studentEmail: `student${i + 1}@example.com`,
    studentPhone: `+91 90000${Math.floor(Math.random() * 9e4 + 1e4)}`,
...
const adminPaymentsController = {
    ...
    try {
      ...
      if (search) {
        filtered = filtered.filter(
          (p) => p.studentName.toLowerCase().includes(search) || p.studentEmail.toLowerCase().includes(search) || p.courseName.toLowerCase().includes(search) || p.transactionId.toLowerCase().includes(search) || p.invoiceNumber.toLowerCase().includes(search)
        );
      }
      if (status) {
  ...
  getPaymentsSummary: async (req, res) => {
    try {
      ...
      const todaysRevenue = successfulPayments.filter((p) => new Date(p.paymentDate) >= today).reduce((sum, p) => sum + p.finalAmount, 0);
      const thisMonth = new Date(today.getFullYear(), today.getMonth(), 1);
      const thisMonthRevenue = successfulPayments.filter((p) => new Date(p.paymentDate) >= thisMonth).reduce((sum, p) => sum + p.finalAmount, 0);
      const summary = {
        totalRevenue,
        todaysRevenue,
        thisMonthRevenue,
  ...
  getPaymentsRevenue: async (req, res) => {
    try {
      ...
      const data = months.slice(0, (/* @__PURE__ */ new Date()).getMonth() + 1).map((month) => ({
        name: month,
