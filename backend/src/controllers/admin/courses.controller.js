...
const courseTitles = [
  ...
  "MBA in a Box: Business Lessons",
  "React Native: The Practical Guide",
  "Data Science and Deep Learning",
  "Illustrator CC 2024 Masterclass",
...
let mockCourses = Array.from({ length: 45 }).map((_, i) => {
  ...
  const price = Math.floor(Math.random() * 5e3) + 1e3;
  ...
  return {
    ...
    instructor: {
      ...
      phone: `+91 99887${Math.floor(1e4 + Math.random() * 9e4)}`,
    ...
    students: Math.floor(Math.random() * 5e3),
    rating: (Math.random() * 2 + 3).toFixed(1),
    createdDate: new Date(Date.now() - Math.random() * 1e10).toISOString(),
    updatedDate: (/* @__PURE__ */ new Date()).toISOString(),
    // Additional Detail Tabs Data
    analytics: {
      completionRate: Math.floor(Math.random() * 40) + 40,
      // 40-80%
      revenue: Math.floor(Math.random() * 5e5),
      certificatesIssued: Math.floor(Math.random() * 2e3)
    },
    curriculum: [
    ...
    reviews: Array.from({ length: 3 }).map((_2, j) => ({
      id: `R${i}-${j}`,
    ...
    activityLog: [
      { id: 1, action: "Course Created", date: new Date(Date.now() - 5e9).toISOString() },
      { id: 2, action: "Submitted for Review", date: new Date(Date.now() - 2e9).toISOString() },
      { id: 3, action: `Status changed to ${status}`, date: (/* @__PURE__ */ new Date()).toISOString() }
    ]
  };
});
const adminCoursesController = {
    ...
    try {
      ...
      if (search) {
        ...
        filtered = filtered.filter(
          (c) => c.title.toLowerCase().includes(lowerSearch) || c.instructor.name.toLowerCase().includes(lowerSearch)
        );
      }
      if (statusFilter !== "All") {
  ...
  getPendingCourses: async (req, res) => {
    try {
      ...
      const stats = {
        ...
        approvedToday: mockCourses.filter((c) => c.status === "Published" && new Date(c.updatedDate).toDateString() === (/* @__PURE__ */ new Date()).toDateString()).length,
        rejectedToday: mockCourses.filter((c) => c.status === "Rejected" && new Date(c.updatedDate).toDateString() === (/* @__PURE__ */ new Date()).toDateString()).length,
        totalPendingReviews: pending.length
        // Can be different if we have different review queues, but same for now
      };
      const startIndex = (page - 1) * limit;
  ...
  updateCourseStatus: async (req, res) => {
    try {
      ...
      course.updatedDate = (/* @__PURE__ */ new Date()).toISOString();
