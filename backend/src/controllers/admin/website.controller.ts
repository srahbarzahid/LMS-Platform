...
export const adminWebsiteController = {
  // GET /api/admin/content
  getWebsiteContent: async (req: Request, res: Response) => {
    try {
  ...
  updateWebsiteContent: async (req: Request, res: Response) => {
    try {
      const { hero, footer, faqs } = req.body;
      
      if (hero) mockWebsiteContent.hero = hero;
      if (footer) mockWebsiteContent.footer = footer;
      if (faqs) mockWebsiteContent.faqs = faqs;
