...
const studentOffersController = {
  // Get automatic offers
  getAutomaticOffers: async (req, res) => {
    try {
      const automaticOffers = offers.filter(
        (o) => (o.offerType === "Course Offer" || o.offerType === "Category Offer") && o.status === "Active"
      );
      res.status(200).json({
        success: true,
