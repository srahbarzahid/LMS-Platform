...
const adminUsersService = {
  ...
  _students: Array.from({ length: 45 }).map((_, i) => ({
    ...
    phone: `+91 98765${Math.floor(1e4 + Math.random() * 9e4)}`,
    enrolledCourses: Math.floor(Math.random() * 5) + 1,
    progress: Math.floor(Math.random() * 100),
    status: Math.random() > 0.8 ? "Blocked" : Math.random() > 0.9 ? "Inactive" : "Active",
    joinedDate: new Date(Date.now() - Math.random() * 1e10).toISOString().split("T")[0],
    profileImage: null
  })),
  _instructors: Array.from({ length: 25 }).map((_, i) => ({
    ...
    revenue: Math.floor(Math.random() * 1e5),
  ...
  createInstructor(data) {
    const newInstructor = {
      id: `INS-${Math.floor(1e4 + Math.random() * 9e4)}`,
      name: data.name,
      email: data.email,
