export class CertificateService {
  ...
  static async getInstructorCertificates(instructorId: string) {
    return this.mockCertificates;
  }

  static async getCertificateDetails(certificateId: string, instructorId: string) {
    const cert = this.mockCertificates.find(c => c.certificateId === certificateId || c.id === certificateId);
    if (!cert) return null;
    return cert;
  }

  static async getCertificateProgress(certificateId: string, instructorId: string) {
    const cert = this.mockCertificates.find(c => c.certificateId === certificateId || c.id === certificateId);
    if (!cert) return null;
  ...
  static async getCertificateTimeline(certificateId: string, instructorId: string) {
    const cert = this.mockCertificates.find(c => c.certificateId === certificateId || c.id === certificateId);
    if (!cert) return null;

    if (cert.certificateStatus === 'Pending') {
      return this.mockTimeline.map((item, index) => {
        if (index > 2) return { ...item, status: 'pending', date: null };
        return item;
      });
    }
