...
export interface UserTableProps {
  ...
  onAction: (action: string, ids: string[], payload?: any) => void;
...
const UserTable = ({
  ...
  filters, entityName = 'Users', detailPathPrefix
}: UserTableProps) => {
                  ...
                  <td className="p-4 text-right">
                    <div className="relative group inline-block">
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 flex flex-col py-2">
                        {detailPathPrefix && (
                           <Link to={`${detailPathPrefix}/${row.id}`} className="px-4 py-2 text-sm text-heading hover:bg-gray-50 font-medium text-left">View Details</Link>
                        )}
                        <button onClick={() => { setSelectedUserForAction(row.id); setStatusModalOpen(true); }} className="px-4 py-2 text-sm text-heading hover:bg-gray-50 font-medium text-left">Change Status</button>
                        <button onClick={() => { setSelectedUserForAction(row.id); setResetPasswordModalOpen(true); }} className="px-4 py-2 text-sm text-heading hover:bg-gray-50 font-medium text-left">Reset Password</button>
      ...
      {resetPasswordModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setResetPasswordModalOpen(false); setNewPassword(''); setShowPassword(false); }} className="px-4 py-2 font-bold text-caption hover:text-heading transition-colors">Cancel</button>
              <button onClick={() => {
