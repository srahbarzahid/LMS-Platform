        ...
        <tr style="border-bottom: 1px solid #e5e7eb; background-color: ${i % 2 === 0 ? "#ffffff" : "#f9fafb"};">
          ${columns
            .map((c) => {
              ...
              if (c.key === "price") {
                cellData = cellData ? `₹${cellData}` : "Free";
              }
            ...
            {selectedIds.length > 0 && (
              <div className="flex items-center gap-2 border-l border-border dark:border-neutral-800 pl-3">
                ...
                <select
                  ...
                  className="px-3 py-1.5 bg-white dark:bg-neutral-800 border border-border dark:border-neutral-700 text-sm font-bold rounded-lg outline-none hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors cursor-pointer"
                >
                            ...
                            >
                              {detailPathPrefix && (
                                <Link
                                  ...
                                  className="px-4 py-2 text-sm text-heading dark:text-white hover:bg-gray-50 dark:hover:bg-neutral-700 font-medium text-left flex items-center gap-2"
                                  onClick={() => setOpenDropdownId(null)}
                                >
                                  <Eye className="w-4 h-4 text-caption" /> View Details
                                </Link>
                              )}
                              <button
                                onClick={() => {
                                  onAction("approve", [row.id]);
                                  setOpenDropdownId(null);
                                }}
                                className="px-4 py-2 text-sm text-heading dark:text-white hover:bg-gray-50 dark:hover:bg-neutral-700 font-medium text-left flex items-center gap-2"
                              >
                              ...
                              <button
                                onClick={() => {
                                  onAction(row.featured ? "unfeature" : "feature", [row.id]);
                                  setOpenDropdownId(null);
                                }}
                                className="px-4 py-2 text-sm text-heading dark:text-white hover:bg-gray-50 dark:hover:bg-neutral-700 font-medium text-left flex items-center gap-2"
                              >
                                <Star className={`w-4 h-4 ${row.featured ? "text-gray-400" : "text-yellow-500"}`} />{" "}
                                {row.featured ? "Unfeature Course" : "Feature Course"}
                              </button>
                              ...
                              <button
                                ...
                                className="px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 font-bold text-left flex items-center gap-2"
                              >
