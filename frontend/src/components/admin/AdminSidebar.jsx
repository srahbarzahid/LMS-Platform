...
const AdminSidebar = () => {
  ...
  return <div className="w-64 bg-white border-r border-border h-screen flex flex-col fixed left-0 top-0 overflow-y-auto">
      {
    /* Brand */
  }
    ...
    /* Navigation */
      ...
      <div className="p-4 flex-1">
        {SIDEBAR_SECTIONS.map((section, idx) => <div key={idx} className="mb-6">
    ...
    return <Link
      key={item.name}
      to={item.path}
      className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sm font-medium ${isActive ? "bg-primary text-white shadow-md shadow-primary/20" : "text-body hover:bg-gray-100 hover:text-heading"}`}
    >
