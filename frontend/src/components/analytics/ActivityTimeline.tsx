...
interface TimelineProps {
  activities: { id: number; type: string; message: string; time: string; icon: string }[];
}

const ActivityTimeline: React.FC<TimelineProps> = ({ activities }) => {
  return (
          ...
          return (
            <div key={activity.id} className="flex gap-4 relative">
              {index !== activities.length - 1 && (
                <div className="absolute top-8 left-[11px] bottom-[-24px] w-[2px] bg-[#f3f4f6]"></div>
              )}
