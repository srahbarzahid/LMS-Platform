...
export const ChartCard = ({ title, children, action }: { title: string, children: React.ReactNode, action?: React.ReactNode }) => (
  <div className="bg-white rounded-2xl border border-[#ff6b00]/10 shadow-[0_4px_20px_-5px_rgba(255,107,0,0.15)] p-6 flex flex-col h-full hover:shadow-[0_8px_30px_-5px_rgba(255,107,0,0.25)] hover:-translate-y-1 transition-[box-shadow,transform] duration-300">
...
// Colors: Primary Orange, Info, Success, Warning, Purple (for variety if needed)
...
export const CustomLineChart = ({ data, xKey, yKey, color = '#ff6b00' }: any) => (
...
export const CustomVerticalBarChart = ({ data, xKey, yKey }: any) => (
  <ResponsiveContainer width="100%" height={300}>
...
export const CustomHorizontalBarChart = ({ data, xKey, yKey, color = '#ff6b00' }: any) => (
  <ResponsiveContainer width="100%" height={300}>
...
export const CustomStackedHorizontalBarChart = ({ data, yKey, keys, colors }: any) => (
  <ResponsiveContainer width="100%" height={300}>
    <BarChart data={data} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
      ...
      {keys.map((key: string, index: number) => (
        <Bar key={key} dataKey={key} stackId="a" fill={colors[index]} radius={
          index === 0 ? [0, 0, 0, 0] : index === keys.length - 1 ? [0, 4, 4, 0] : [0, 0, 0, 0]
        } barSize={30} />
      ))}
    </BarChart>
  </ResponsiveContainer>
...
export const CustomDonutChart = ({ data, nameKey, dataKey }: any) => (
...
export const CircularProgressChart = ({ value, label, subtext, color = '#ff6b00' }: any) => {
  const data = [
