...
const mockQuizResult = {
  ...
  questions: [
    ...
    { 
      ...
      correct: 3,
      selected: 2 // Incorrect answer picked
    },
    { 
      id: 3, 
    ...
    <div className="space-y-8 pb-8 max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
        {mockQuizResult.questions.map((q, idx) => {
          ...
          return (
            <div key={q.id} className="bg-white rounded-3xl p-6 lg:p-8 border border-border shadow-sm">
              ...
              <div className="space-y-3">
                {q.options.map((option, optIdx) => {
                  ...
                  let optionStyle = "border-gray-200 bg-gray-50 text-gray-500"; // Default unused
                  ...
                  return (
                    <div key={optIdx} className={`w-full p-4 rounded-xl border flex items-center gap-4 ${optionStyle}`}>
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${
                        (isSelected && isActuallyCorrect) ? 'border-emerald-500 bg-emerald-500 text-white' :
                        (isSelected && !isActuallyCorrect) ? 'border-red-500 bg-red-500 text-white' :
                        (!isSelected && isActuallyCorrect) ? 'border-emerald-500 text-emerald-500' :
                        'border-gray-300'
                      }`}>
                      ...
                      {!isSelected && isActuallyCorrect && (
                        <span className="ml-auto text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-200">
                          Correct Answer
                        </span>
                      )}
                      {isSelected && !isActuallyCorrect && (
                        <span className="ml-auto text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded border border-red-200">
                          Your Answer
                        </span>
                      )}
                      {isSelected && isActuallyCorrect && (
