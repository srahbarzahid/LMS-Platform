...
const InstructorReviews = () => {
  ...
  const handleReplySubmit = (id) => {
    ...
    setReviews((prev) => prev.map(
      (r) => r.id === id ? { ...r, reply: replyText[id] } : r
    ));
    setShowReplyForm((prev) => ({ ...prev, [id]: false }));
  };
  const filteredReviews = reviews.filter(
    (r) => r.course.toLowerCase().includes(searchTerm.toLowerCase()) || r.text.toLowerCase().includes(searchTerm.toLowerCase()) || r.student.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  />
    ...
    /* Reviews List */
      ...
      {loading ? <div className="flex py-20 items-center justify-center">
          ...
          {filteredReviews.length > 0 ? filteredReviews.map((review) => <div key={review.id} className="bg-white rounded-2xl border border-border shadow-sm p-6 flex flex-col gap-4">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => <Star key={star} className={`w-4 h-4 ${star <= review.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`} />)}
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 pt-2">
                {!review.reply && !showReplyForm[review.id] && <button
  ...
  >
    ...
    /* Reply Form */
              ...
              {showReplyForm[review.id] && !review.reply && <div className="bg-gray-50 border border-border rounded-xl p-4 mt-2 animate-fade-in flex gap-4 items-start">
  ...
  >
    ...
    /* Existing Reply */
              ...
              {review.reply && <div className="bg-orange-50/50 border border-orange-100 rounded-xl p-4 mt-2 flex gap-4 items-start">
...
var stdin_default = InstructorReviews;
export {
