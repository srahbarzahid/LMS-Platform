...
const InstructorDashboard = () => {
  ...
  >
              ...
              {revenueDropdownOpen && <>
                  <div className="fixed inset-0 z-40" onClick={() => setRevenueDropdownOpen(false)} />
                  <div className="absolute right-0 top-full mt-1 w-full bg-white border border-border rounded-lg shadow-lg z-50 overflow-hidden">
                    {["Last 7 Days", "Last 30 Days", "This Year"].map((opt) => <div
    key={opt}
    onClick={() => {
      setRevenueTime(opt);
    ...
    /* Mock Donut Chart using Conic Gradient */
            ...
            <div className="space-y-4 flex-1 w-full">
               ...
               <div>
                 ...
                 <div className="w-full bg-gray-100 rounded-full h-2">
                   <div className="bg-orange-400 h-2 rounded-full" style={{ width: `${100 - stats.completionRate}%` }} />
                 </div>
               </div>
            </div>
          </div>
...
var stdin_default = InstructorDashboard;
const StatCard = ({ title, value, icon, trend }) => <div className="bg-white p-6 rounded-2xl border border-border shadow-sm flex items-start gap-4">
    ...
    <div>
      ...
      {trend && <p className={`text-xs font-bold mt-1 ${trend.startsWith("+") ? "text-green-600" : "text-red-500"}`}>
          {trend} this month
        </p>}
    </div>
  </div>;
const QuickAction = ({ icon, title, to }) => <Link to={to} className="flex flex-col items-center gap-3 p-4 rounded-xl border border-border bg-gray-50/50 hover:bg-orange-50 hover:border-orange-200 transition-colors text-center group">
