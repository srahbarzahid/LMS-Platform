...
import { 
  Search,
  Filter,
  Users, CheckCircle, Activity, BarChart, Award, FileText,
  Eye, BarChart2, Folder, Clock, ChevronDown
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const CustomDropdown = ({ icon: Icon, options, value, onChange, placeholder }: any) => {
  ...
  return (
    <div className="relative shrink-0" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-full min-w-[160px] bg-white border border-border rounded-xl shadow-lg z-50 overflow-hidden py-1.5 transform origin-top transition-all">
          <button
            onClick={() => {
              onChange('');
              setIsOpen(false);
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          <div key={idx} className="bg-white p-4 rounded-2xl border border-border shadow-sm flex flex-col justify-center items-center text-center">
                  ...
                  <tr key={student.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="py-4 px-4">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-bold ${
                        student.courseStatus === 'Active' ? 'bg-green-100 text-green-700' : 
                        student.courseStatus === 'Completed' ? 'bg-purple-100 text-purple-700' : 
                    ...
                    <td className="py-4 px-4">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-bold ${
                        student.certificateStatus === 'Generated' ? 'bg-blue-100 text-blue-700' : 
                        student.certificateStatus === 'Eligible' ? 'bg-yellow-100 text-yellow-700' : 
                        'bg-gray-100 text-gray-700'
                      }`}>
