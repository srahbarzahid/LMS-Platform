...
const InstructorAccountSecurityTab = () => {
  ...
  const fetchSessions = async () => {
    ...
    try {
      const res = await apiClient.get(normalizeApiPath("/instructor/settings/sessions"));
      const data = res.data;
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await apiClient.post(normalizeApiPath("/instructor/settings/change-password"), {
        currentPassword,
        newPassword,
        confirmPassword
      });
  ...
  const handleLogoutSession = async (sessionId) => {
    try {
      const res = await apiClient.delete(normalizeApiPath(`/instructor/settings/sessions/${sessionId}`));
      const data = res.data;
      if (data.status === "success") {
  ...
  const handleLogoutAllOtherSessions = async () => {
    ...
    try {
      const res = await apiClient.delete(normalizeApiPath("/instructor/settings/sessions"));
      const data = res.data;
      if (data.status === "success") {
  ...
  const handleDeactivateAccount = async () => {
    ...
    try {
      const res = await apiClient.post(normalizeApiPath("/instructor/settings/deactivate"), {
        confirmPassword: deactivatePassword
      });
      const data = res.data;
      if (data.status === "success") {
      ...
      toast.error(getApiErrorMessage(err, "Error deactivating account."));
    } finally {
...
var stdin_default = InstructorAccountSecurityTab;
export {
