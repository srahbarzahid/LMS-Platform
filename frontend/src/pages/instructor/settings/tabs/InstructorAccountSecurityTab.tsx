...
import { 
  ...
  AlertTriangle, CheckCircle2, RefreshCw, KeyRound, AlertOctagon, UserX
} from 'lucide-react';
...
const InstructorAccountSecurityTab: React.FC = () => {
  // Password state
  ...
  const fetchSessions = async () => {
    ...
    try {
      const res = await fetch('/api/instructor/settings/sessions', {
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/instructor/settings/change-password', {
        method: 'POST',
        headers: {
  ...
  const handleLogoutSession = async (sessionId: string) => {
    try {
      const res = await fetch(`/api/instructor/settings/sessions/${sessionId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${localStorage.getItem('token') || ''}` },
      });
      const data = await res.json();
  ...
  const handleDeactivateAccount = async () => {
    ...
    try {
      const res = await fetch('/api/instructor/settings/deactivate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token') || ''}`,
      ...
      const data = await res.json();
      if (res.ok && data.status === 'success') {
        toast.success(data.message || 'Instructor account deactivated.');
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-caption">Manage password security, channel verification, active login sessions, and account deactivation.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-6">
        <div className="flex items-center gap-3 pb-4 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-caption">Keep your instructor portal secure with a complex password.</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-4">
        <div>
          ...
          <p className="text-xs text-caption">Verification status of channels used for notifications and security resets.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
          <div>
            ...
            <p className="text-xs text-caption">Devices currently logged into your instructor account.</p>
          </div>
          {sessions.length > 1 && (
            <button
      ...
      <div className="bg-red-50/50 rounded-2xl border border-red-200 p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-red-600 text-base">Deactivate Instructor Account</h4>
            <p className="text-xs text-red-500">Temporarily suspend your instructor profile, course creation, and teaching dashboard.</p>
          </div>
        </div>

        <p className="text-xs text-caption leading-relaxed">
          Deactivating your account will unpublish your courses from public catalog search and pause new enrollments. Enrolled students retain access to existing materials while your account is in soft-delete state.
        </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl animate-fade-in">
            <div className="flex items-center gap-3 text-red-600">
              ...
              <h4 className="font-bold text-lg">Confirm Instructor Account Deactivation</h4>
            </div>
            
            <p className="text-sm text-caption leading-relaxed">
              Deactivating your account will suspend your instructor dashboard, unpublish your courses from public search, and pause new enrollments. Your existing courses and student submission history will be preserved safely.
            </p>
...
export default InstructorAccountSecurityTab;
