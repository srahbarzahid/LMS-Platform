...
import { 
  Award, Search, Filter, 
  Eye, BarChart2, CheckCircle, Clock, ChevronDown
} from 'lucide-react';
...
const InstructorCertificates = () => {
  ...
  useEffect(() => {
    ...
    const fetchCertificates = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // If mock fails, fallback to hardcoded mock data for now
    ...
    // Simulate loading since mock might be fast or missing in dev
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-white p-5 rounded-2xl border border-border shadow-sm flex items-center justify-between">
            <div>
                  ...
                  <tr key={cert.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="py-4 px-4">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-bold ${
                        cert.certificateStatus === 'Issued' ? 'bg-green-100 text-green-700' : 
                        cert.certificateStatus === 'Eligible' ? 'bg-blue-100 text-blue-700' : 
