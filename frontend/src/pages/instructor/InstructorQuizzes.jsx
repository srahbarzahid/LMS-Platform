...
const InstructorQuizzes = () => {
  ...
  const [selectedItemId, setSelectedItemId] = useState("i3");
  ...
  >
    ...
    return <div
      key={item.id}
      onClick={() => {
        if (isQuiz) setSelectedItemId(item.id);
      }}
    ...
    /* Questions Section */
          ...
          <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              {questions.map((q, idx) => <div key={q.id} className="border border-border rounded-xl overflow-hidden bg-gray-50/30">
  ...
  />
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {q.options.map((opt, optIdx) => <div key={optIdx} className="flex items-center gap-3">
                        <div
  ...
  >
                          {q.correctOption === optIdx && <div className="w-2 h-2 bg-white rounded-full" />}
                        </div>
                        <input
    type="text"
    value={opt}
  ...
  >
                    ...
                    {q.options.length < 6 && <button onClick={() => addOption(q.id)} className="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1 mt-2 cursor-pointer ml-8">
                        <Plus className="w-4 h-4" /> Add Option
                      </button>}
                  </div>
                </div>)}

              {questions.length === 0 && <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
