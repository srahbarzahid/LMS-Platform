...
const CourseDetails = () => {
  ...
  useEffect(() => {
    // Fetch reviews from backend to display published ones for this specific course
    const fetchReviews = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/admin/reviews?courseId=${courseData.id}`, { withCredentials: true });
        if (res.data.success) {
          const publishedCourseReviews = res.data.data.filter((r: any) => r.status === 'Published');
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen pb-20 relative">
      ...
      <section className="bg-heading text-white pt-32 pb-24 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/2 h-full opacity-10 bg-primary skew-x-12 translate-x-32"></div>
            ...
            <div className="bg-white p-8 md:p-10 rounded-2xl border border-border shadow-sm">
              ...
              <div className="space-y-4">
                {courseModules.map((module) => (
                  <div key={module.id} className="border border-border rounded-xl overflow-hidden transition-all bg-white">
                    <div 
                    ...
                    {expandedModule === module.id && (
                      <div className="bg-[#f8f9fa] px-6 py-6 border-t border-border space-y-3">
            ...
            <div className="bg-white p-8 md:p-10 rounded-2xl border border-border shadow-sm">
              ...
              <div className="flex items-center gap-6 mb-8 border-b border-border pb-8">
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-1 mb-1">
                    {[1, 2, 3, 4, 5].map(star => (
                      <Star key={star} className={`w-5 h-5 ${star <= 5 ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`} />
                    ))}
                  </div>
              ...
              <div className="space-y-6">
                {currentReviews.length > 0 ? (
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-1 mb-3">
                            {[1, 2, 3, 4, 5].map(star => (
                              <Star key={star} className={`w-3.5 h-3.5 ${star <= review.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`} />
                            ))}
                          </div>
                          {review.reviewTitle && (
                            <h5 className="font-bold text-heading text-sm mb-2">{review.reviewTitle}</h5>
                          )}
                  ...
                  <div className="text-center py-8 text-body bg-gray-50 rounded-xl border border-border">
