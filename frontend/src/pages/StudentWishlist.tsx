...
const StudentWishlist = () => {
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      {wishlist.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-border text-center shadow-sm flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {wishlist.map((course) => (
            <div key={course.id} className="bg-white rounded-3xl border border-border overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group relative hover:-translate-y-1 hover:border-primary/30">
              <div className="relative h-48 overflow-hidden bg-gray-100">
                ...
                {course.tag && (
                  <span className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-heading px-3 py-1 text-[10px] uppercase font-bold rounded-lg shadow-sm tracking-wider">
                    {course.tag}
                  </span>
                )}
              </div>
              ...
              <div className="p-6 flex-grow flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  {course.category && (
                    <span className="text-orange-600 text-[10px] font-bold uppercase tracking-wider bg-orange-50 px-2 py-0.5 rounded">
                      {course.category}
                    </span>
                  )}
                ...
                {course.author && (
                  <p className="text-xs text-caption mb-4">Instructor: <span className="font-medium text-heading">{course.author}</span></p>
                )}
