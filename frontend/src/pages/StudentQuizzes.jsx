...
const StudentQuizzes = () => {
  ...
  const avgScore = completedScores.length > 0 ? Math.round(completedScores.reduce((acc, q) => acc + q.score / q.totalMarks * 100, 0) / completedScores.length) : 0;
  ...
  />
    ...
    /* Quiz List */
      ...
      {filteredQuizzes.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredQuizzes.map((quiz) => <div key={quiz.id} className="bg-white rounded-3xl border border-border shadow-sm p-6 flex flex-col hover:shadow-lg transition-shadow">
    ...
    /* Actions */
              ...
              <div className="pt-4 border-t border-border mt-auto">
                {quiz.status === "Not Started" && <button
  ...
  >
                ...
                {quiz.status === "In Progress" && <button
    onClick={() => handleAction(quiz.id, quiz.status)}
  ...
  >
                ...
                {(quiz.status === "Completed" || quiz.status === "Expired") && <button
    onClick={() => handleAction(quiz.id, quiz.status)}
  ...
  >
    ...
    <div className="bg-white rounded-3xl border border-border p-12 text-center shadow-sm flex flex-col items-center justify-center">
          ...
          <p className="text-body max-w-md mx-auto mb-8">
            {searchQuery ? "Try adjusting your search or filters to see more results." : "Enroll in courses that offer quizzes to test your knowledge."}
          </p>
...
var stdin_default = StudentQuizzes;
export {
