      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {wishlist.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {wishlist.map((course) => (
              <div key={course.id} className="bg-white rounded-3xl border border-border overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col group relative hover:-translate-y-1">
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  ...
                  {course.tag && (
                    <span className="absolute top-4 left-4 bg-white text-heading px-3 py-1 text-xs font-bold rounded-full shadow-sm">
                      {course.tag}
                    </span>
                  )}
                </div>
                <div className="p-6 flex-grow flex flex-col">
                  <div className="flex items-center justify-between mb-3">
                    {course.category && (
                      <span className="text-orange-500 text-xs font-bold uppercase tracking-wider bg-orange-50 px-2 py-1 rounded-md">{course.category}</span>
                    )}
                  ...
                  {course.author && (
                    <p className="text-xs text-body mb-4">By <span className="font-medium text-heading">{course.author}</span></p>
                  )}
