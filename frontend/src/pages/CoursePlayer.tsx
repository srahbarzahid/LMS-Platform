...
import { 
  PlayCircle, CheckCircle2, Lock, ChevronDown, ChevronUp, Search, Bell, 
  Menu, User, FileText, Download, Star, StarHalf, MonitorPlay, 
  ArrowLeft, ArrowRight, Award, FileCode, UploadCloud, Clock
} from 'lucide-react';
...
const CoursePlayer = () => {
  ...
  const selectLesson = (lesson: any, moduleId: number) => {
    if (lesson.locked) {
  ...
  return (
    <div className="h-full bg-[#f8f9fa] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto w-full lg:w-[70%] custom-scrollbar pb-24 lg:pb-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 self-start lg:self-center">
                ...
                {!activeLesson.completed ? (
                  ...
                  <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-4 py-2 rounded-full text-sm font-medium border border-emerald-100">
          ...
          <div className="p-4 lg:p-8">
            <div className="flex overflow-x-auto custom-scrollbar border-b border-border gap-8 mb-8 pb-2">
              {['overview', 'resources', 'assignments', 'quiz', 'reviews'].map(tab => (
                <button
                  key={tab}
                      ...
                      {[5,4,3,2,1].map(stars => (
                        <div key={stars} className="flex items-center gap-3 text-sm">
                          ...
                          <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                            <div className="h-full bg-orange-400" style={{width: stars === 5 ? '75%' : stars === 4 ? '20%' : '0%'}}></div>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className={`fixed inset-y-0 right-0 z-30 w-80 bg-white border-l border-border transform transition-transform duration-300 ease-in-out lg:relative lg:w-[30%] lg:min-w-[320px] lg:translate-x-0 flex flex-col ${showMobileSidebar ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="p-4 border-b border-border shrink-0 bg-white">
            ...
            {progressPercentage >= 100 && (
              <div className="mt-3 flex items-center gap-2 text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-2 rounded-lg">
                        ...
                        >
                          <div className="shrink-0 mt-0.5">
                            {lesson.completed ? (
                              ...
                              <PlayCircle className="w-5 h-5 text-caption" />
                            )}
                          </div>
                          <div>
      ...
      {toastMessage && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-full shadow-2xl z-50 flex items-center gap-3 animate-fade-in-up">
