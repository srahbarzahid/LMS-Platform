...
type QuizState = 'PRE_QUIZ' | 'ACTIVE' | 'SUBMIT_CONFIRM' | 'RESULT';

const QuizPlayer = () => {
  useParams();
  
  const [quizState, setQuizState] = useState<QuizState>('PRE_QUIZ');
  ...
  const handleOptionSelect = (optionIdx: number) => {
    if (showReview) return; // Prevent changing answers in review mode
    setAnswers({
      ...answers,
  ...
  // ACTIVE OR REVIEW OR SUBMIT_CONFIRM STATE
  return (
    <div className="min-h-screen bg-[#f8f9fa] flex flex-col">
      ...
      <header className="h-16 bg-white border-b border-border flex items-center justify-between px-4 lg:px-8 shrink-0 sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4">
          {showReview ? (
            <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-lg text-sm font-bold border border-orange-200">Review Mode</span>
          ) : (
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-bold border ${timeLeft < 300 ? 'bg-red-50 text-red-600 border-red-200 animate-pulse' : 'bg-gray-100 text-heading border-border'}`}>
              <Clock className="w-4 h-4" />
              <span>{formatTime(timeLeft)}</span>
            </div>
          )}
          {!showReview && (
            <button onClick={handleSubmitClick} className="hidden sm:block px-5 py-2 bg-primary text-white text-sm font-bold rounded-lg hover:bg-secondary transition-colors">
              Submit Quiz
            </button>
          )}
        </div>
        ...
        <div className="flex-1 p-4 lg:p-8 flex flex-col">
          <div className="bg-white rounded-3xl p-6 lg:p-10 border border-border shadow-sm flex-1 flex flex-col">
            ...
            <div className="flex justify-between items-center mb-8 pb-4 border-b border-border">
              ...
              {showReview && (
                <span className={`text-sm font-bold px-3 py-1 rounded-lg ${answers[currentQuestionIdx] === currentQuestion.correct ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                ...
                return (
                  ...
                  >
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                      (isSelected && !showReview) ? 'border-primary' : 
                      (showReview && idx === currentQuestion.correct) ? 'border-emerald-500 bg-emerald-500 text-white' :
                      (showReview && isSelected && idx !== currentQuestion.correct) ? 'border-red-500 bg-red-500 text-white' :
                      'border-gray-300'
                    }`}>
                      {((isSelected && !showReview) || (showReview && idx === currentQuestion.correct)) && !showReview && (
            ...
            <div className="mt-auto flex justify-between items-center pt-6 border-t border-border">
              ...
              {currentQuestionIdx === mockQuizData.questions.length - 1 && !showReview ? (
                <button 
          ...
          <div className="bg-white rounded-3xl border border-border shadow-sm p-6 sticky top-24">
            ...
            <div className="grid grid-cols-4 gap-2 mb-8">
              {mockQuizData.questions.map((q, idx) => {
                ...
                let navStyle = "bg-gray-50 border-gray-200 text-gray-500 hover:border-gray-400"; // Unanswered
            ...
            <div className="space-y-3 mb-8 text-sm">
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded bg-primary/10 border-2 border-primary"></div>
            ...
            {!showReview ? (
               <button onClick={handleSubmitClick} className="w-full sm:hidden px-5 py-3 bg-primary text-white font-bold rounded-xl hover:bg-secondary transition-colors">
                 Submit Quiz
               </button>
            ) : (
              <Link to="/student/quizzes" className="block w-full text-center px-5 py-3 bg-gray-100 text-heading font-bold rounded-xl hover:bg-gray-200 transition-colors">
                 Exit Review
              </Link>
            )}
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
            ...
            <p className="text-body mb-8">
              {Object.keys(answers).length < mockQuizData.questions.length 
                ? `You have ${mockQuizData.questions.length - Object.keys(answers).length} unanswered questions. Are you sure you want to submit?`
