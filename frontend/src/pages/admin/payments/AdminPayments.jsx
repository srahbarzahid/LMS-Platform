...
const AdminPayments = () => {
  ...
  return <div className="space-y-6">
    ...
    /* Stats Cards */
      ...
      {statCards.length > 0 && <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
    ...
    /* Revenue Section */
      ...
      {summary && <div className="bg-white rounded-3xl border border-border shadow-sm overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="p-6 lg:w-2/3 h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={revenueData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fill: "#6b7280", fontSize: 12, fontWeight: 600 }} tickFormatter={(val) => `\u20B9${val / 1e3}k`} />
                <Tooltip
    cursor={{ fill: "#f3f4f6" }}
            ...
            <tbody className="text-sm">
              {loading ? <tr>
                  ...
                  <td colSpan={6} className="p-8 text-center text-caption font-medium">No payments found.</td>
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${row.status === "Paid" ? "bg-emerald-50 text-emerald-700" : row.status === "Pending" ? "bg-yellow-50 text-yellow-700" : row.status === "Failed" ? "bg-red-50 text-red-700" : "bg-gray-100 text-gray-700"}`}>
                        {row.status}
                      </span>
                    </td>
...
var stdin_default = AdminPayments;
export {
