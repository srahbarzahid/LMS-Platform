...
import { 
  Tag, Plus, Search, CheckCircle, Clock, 
  XCircle, Eye, Trash2, X, AlertTriangle, Activity, IndianRupee, ChevronDown
} from 'lucide-react';
...
const CustomDropdown = ({ value, options, onChange, placeholder, className = '', disabled = false }: { value: string, options: {value: string, label: string}[], onChange: (val: string) => void, placeholder: string, className?: string, disabled?: boolean }) => {
...
export default function AdminOffers() {
  ...
  const [statusConfirm, setStatusConfirm] = useState<{id: string, name: string, status: string} | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        usagePerStudent: 1, // Hardcoded to 1 per student globally
      };
      ...
      // Reset form
      setFormData({
        name: '', code: '', offerType: 'Coupon Code', discountType: 'Percentage', discountValue: '',
        applicableType: 'All Courses', targetId: '', targetName: '', minimumPurchaseAmount: '', 
        maximumDiscountAmount: '', usageLimit: '', usagePerStudent: '1', startDate: '', endDate: '', status: 'Active'
      });
    } catch (error) {
                  ...
                  <tr key={offer.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        {offer.discountType === 'Percentage' && offer.maximumDiscountAmount 
                          ? `Up to ₹${offer.maximumDiscountAmount}` : 'No Limit'}
                      </div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col my-8 overflow-hidden border border-gray-100">
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-white">
              <form id="addOfferForm" onSubmit={handleSubmit} className="space-y-6">
                ...
                {formData.offerType === 'Coupon Code' && (
                  <div>
