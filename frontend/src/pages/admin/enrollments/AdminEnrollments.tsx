      ...
      {statCards.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {statCards.map((stat, idx) => {
            ...
            return (
              <div key={idx} className="bg-white p-4 rounded-xl border border-border flex items-center gap-4">
        ...
        <div className="p-6 border-b border-border bg-gray-50/50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <CustomDropdown
              ...
              onChange={(val) => { setFilters({...filters, certStatus: val}); setPage(1); }}
                  ...
                  <tr key={row.id} className="border-b border-border/50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                        row.status === 'Active' ? 'bg-indigo-50 text-indigo-700' :
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                        row.certificateStatus === 'Issued' ? 'bg-emerald-50 text-emerald-700' :
                        row.certificateStatus === 'Eligible' ? 'bg-yellow-50 text-yellow-700' :
                        'bg-gray-100 text-gray-700'
                      }`}>
