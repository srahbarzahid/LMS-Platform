...
import { 
  ShieldCheck, Lock, Eye, EyeOff, Smartphone, Laptop, Globe, LogOut, 
  AlertTriangle, CheckCircle2, XCircle, RefreshCw, KeyRound, ShieldAlert
} from 'lucide-react';
...
const AccountSecurityTab: React.FC = () => {
  ...
  const [selectedSessionToDelete, setSelectedSessionToDelete] = useState<string | null>(null);
  ...
  const handleResendVerification = async (type: 'email' | 'mobile') => {
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-caption">Manage password security, multi-channel verification, and active sessions.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-6">
        <div className="flex items-center gap-3 pb-4 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-caption">Ensure your account is using a strong, unique password.</p>
          </div>
        </div>
            ...
            {newPassword && (
              <div className="mt-2 space-y-1">
                ...
                <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-300 ${strengthInfo.bg} ${strengthInfo.width}`}></div>
                </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-4">
        <div>
          ...
          <p className="text-xs text-caption">Active authentication and alert recovery channels.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="border border-border rounded-xl p-4 bg-gray-50/50 flex justify-between items-center">
            <div>
              <div className="text-xs text-caption uppercase tracking-wider font-bold">Email Channel</div>
          ...
          <div className="border border-border rounded-xl p-4 bg-gray-50/50 flex justify-between items-center">
            <div>
              <div className="text-xs text-caption uppercase tracking-wider font-bold">Mobile SMS Channel</div>
              <div className="text-sm font-bold text-heading mt-0.5 inline-flex items-center gap-1.5 text-amber-600">
                <AlertTriangle className="w-4 h-4" /> Pending / Action Required
              </div>
            </div>
            <button
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
          <div>
            <h4 className="font-bold text-heading text-base">Active Sessions</h4>
            <p className="text-xs text-caption">Devices currently logged into your admin account.</p>
          </div>
          {sessions.length > 1 && (
            <button
        ...
        {loadingSessions ? (
          ...
          <div className="space-y-3">
            {sessions.map((sess) => (
              <div key={sess.id} className="border border-border rounded-xl p-4 flex items-center justify-between gap-4 hover:border-gray-300 transition-colors">
                <div className="flex items-center gap-3.5">
                  <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-heading shrink-0">
                    {sess.device.toLowerCase().includes('mac') || sess.device.toLowerCase().includes('pc') || sess.device.toLowerCase().includes('windows') ? (
                  ...
                  <div>
                    <div className="text-sm font-bold text-heading flex items-center gap-2">
                      {sess.device}
                      {sess.isCurrent && (
                        <span className="px-2 py-0.5 bg-green-100 text-green-700 text-[10px] font-extrabold rounded-full uppercase tracking-wider">
                          Current Session
                        </span>
                      )}
                    </div>
      ...
      {/* 4. Admin Lockout Guard Banner */}
      <div className="p-4 rounded-2xl bg-gray-50 border border-border flex items-start gap-3">
        <ShieldAlert className="w-5 h-5 text-gray-500 shrink-0 mt-0.5" />
        <div className="text-xs text-caption leading-relaxed">
          <strong className="text-heading block mb-0.5">System Security Note</strong>
          Self-service account deactivation is disabled for Admin roles to prevent accidental platform lockouts and preserve administrative accountability.
        </div>
      </div>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl animate-fade-in">
            ...
            <p className="text-sm text-caption">Are you sure you want to update your password? You will need to use your new password next time you log in.</p>
      ...
      {selectedSessionToDelete && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl animate-fade-in">
            <h4 className="font-bold text-heading text-lg">Revoke Session</h4>
            <p className="text-sm text-caption">Are you sure you want to log out this active session? The device will be disconnected immediately.</p>
            <div className="flex justify-end gap-3 pt-2">
              <button
      ...
      {showLogoutAllModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl animate-fade-in">
            <h4 className="font-bold text-heading text-lg">Log Out All Other Sessions</h4>
            <p className="text-sm text-caption">Are you sure you want to disconnect all other active devices? Only your current session will remain active.</p>
            <div className="flex justify-end gap-3 pt-2">
              <button
