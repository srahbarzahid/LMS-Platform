...
import { 
  Users, BookOpen, DollarSign, TrendingUp, TrendingDown, CheckCircle, Star, 
  Download, RefreshCw, Filter, FileText, Activity, AlertTriangle, ChevronDown
} from 'lucide-react';
...
import { 
  ...
  CustomDonutChart, CustomHorizontalBarChart
} from '../../../components/analytics/ChartCards';

const CustomDropdown = ({ value, onChange, options, label }: any) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 top-full right-0 mt-2 w-full min-w-[150px] bg-white border border-gray-100 rounded-xl shadow-xl py-1 overflow-hidden origin-top animate-in fade-in zoom-in-95 duration-200">
...
const KPICard = ({ title, value, growth, description, icon: Icon, colorClass, onClick }: any) => {
  // Use the colorClass to extract the base background color for the decorative blur
  ...
  return (
    ...
    >
      ...
      <div className={`absolute -right-8 -top-8 w-32 h-32 rounded-full opacity-10 ${bgBase} blur-3xl group-hover:scale-150 transition-transform duration-700 pointer-events-none`}></div>
      ...
      <div className="flex items-center gap-2 mt-auto pt-2 relative z-10">
        {growth !== undefined && (
          <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-md ${growth > 0 ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100'}`}>
...
const AdminAnalytics = () => {
  ...
  // State for all 14 datasets
  const [summary, setSummary] = useState<any>(null);
  const [users, setUsers] = useState<any>(null);
  const [revenue, setRevenue] = useState<any>(null);
  const [courses, setCourses] = useState<any>(null);
  const [, setEnrollments] = useState<any>(null);
  ...
  const [, setOffers] = useState<any>(null);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const [
        sumRes, userRes, revRes, courseRes, enrollRes,
        catRes, instRes, payRes, certRes, reviewRes,
        offerRes, actRes, insightRes
      ] = await Promise.all([
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12 print:max-w-none print:m-0 print:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 print:hidden">
        <KPICard title="Total Revenue" value={`$${(summary.totalRevenue.value / 1000).toFixed(1)}k`} growth={summary.totalRevenue.growth} description="Gross revenue generated" icon={DollarSign} colorClass="bg-green-500 text-green-500" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 print:hidden">
        {insights.slice(0,3).map((insight: any) => (
          <div key={insight.id} className={`p-5 rounded-2xl border ${insight.type === 'success' ? 'bg-green-50 border-green-200' : insight.type === 'warning' ? 'bg-orange-50 border-orange-200' : insight.type === 'danger' ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-200'}`}>
      ...
      <div id="pdf-report-container" style={{ display: 'none' }} className="font-sans bg-white p-8 w-[900px]">
        ...
        <div className="flex justify-between items-end border-b-4 border-primary pb-6 mb-8">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-600 font-semibold">
              Time: <span className="font-normal">{new Date().toLocaleTimeString()}</span>
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse rounded-xl overflow-hidden border border-gray-200">
            ...
            <tbody className="divide-y divide-gray-200 bg-gray-50">
              {instructors.map((inst: any, i: number) => (
                <tr key={inst.id} className={i % 2 === 0 ? 'bg-white' : ''}>
        ...
        <div className="mb-12" style={{ pageBreakInside: 'avoid' }}>
          ...
          <table className="w-full text-left border-collapse rounded-xl overflow-hidden border border-gray-200">
            ...
            <tbody className="divide-y divide-gray-200 bg-gray-50">
              {courses.topCourses.map((course: any, idx: number) => (
        ...
        <div className="mt-16 pt-6 border-t border-gray-300 text-center text-gray-500 text-sm" style={{ pageBreakInside: 'avoid' }}>
          <p>© {new Date().getFullYear()} PiTech LMS Platform. All rights reserved.</p>
