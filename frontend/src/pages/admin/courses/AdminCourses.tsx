...
import CourseTable, { type Column } from '../../../components/admin/courses/CourseTable';
...
const AdminCourses = () => {
  ...
  const [confirmModal, setConfirmModal] = useState<{isOpen: boolean, action: string, ids: string[], reason?: string}>({isOpen: false, action: '', ids: []});
  ...
  const executeAction = async (action: string, ids: string[], reason?: string) => {
    try {
      if (action === 'delete') {
  ...
  const columns: Column[] = [
      ...
      render: (val, row) => (
        <div className="flex items-center gap-3">
          <div className="w-12 h-10 rounded-lg bg-gray-200 overflow-hidden flex-shrink-0 relative">
            ...
            {row.featured && (
              <div className="absolute top-0 right-0 bg-yellow-400 p-0.5 rounded-bl-lg">
    ...
    { 
      ...
      label: 'Last Updated',
      render: (val) => (
        <span className="text-caption text-sm">{new Date(val).toLocaleDateString()}</span>
      )
    }
  ];
  ...
  return (
    <div className="p-6 md:p-8 pt-4 max-w-7xl mx-auto space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden transform scale-100 opacity-100 transition-all duration-300">
            <div className={`h-2 w-full ${
              confirmModal.action === 'delete' || confirmModal.action === 'reject' ? 'bg-red-500' :
              confirmModal.action === 'approve' ? 'bg-emerald-500' :
              confirmModal.action === 'feature' ? 'bg-yellow-500' : 'bg-primary'
            }`}></div>
            
            <div className="p-8 text-center flex flex-col items-center">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shadow-inner ${
                confirmModal.action === 'delete' || confirmModal.action === 'reject' ? 'bg-red-50 text-red-500' :
                confirmModal.action === 'approve' ? 'bg-emerald-50 text-emerald-500' :
                confirmModal.action === 'feature' ? 'bg-yellow-50 text-yellow-500' : 'bg-primary/10 text-primary'
              }`}>
              ...
              <div className="flex gap-3 w-full">
                ...
                <button 
                  ...
                  className={`flex-1 px-5 py-3 rounded-xl font-bold text-white transition-colors ${
                    confirmModal.action === 'delete' || confirmModal.action === 'reject' ? 'bg-red-500 hover:bg-red-600' :
                    confirmModal.action === 'approve' ? 'bg-emerald-500 hover:bg-emerald-600' :
                    confirmModal.action === 'feature' ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-primary hover:bg-primary/90'
                  }`}
                >
