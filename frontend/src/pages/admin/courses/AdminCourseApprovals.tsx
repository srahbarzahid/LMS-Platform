...
import { BookOpen, CheckCircle, Clock, XCircle, RefreshCw } from 'lucide-react';
import toast from 'react-hot-toast';
import CourseApprovalTable, { type Column } from '../../../components/admin/courses/CourseApprovalTable';
...
const AdminCourseApprovals = () => {
  ...
  const [confirmModalState, setConfirmModalState] = useState<{isOpen: boolean, courseId: string | null, action: 'approve' | 'reject' | null, reason?: string}>({
    isOpen: false,
    courseId: null,
    action: null
  });

  const [successModalState, setSuccessModalState] = useState<{isOpen: boolean, title: string, message: string}>({
    isOpen: false,
    title: '',
    message: ''
  });
  ...
  const handleAction = (action: string, row: any) => {
    if (action === 'review') {
      ...
      // Direct reject from table without reason is fine, but better to open review modal for reason
      setSelectedCourse(row);
      setReviewModalOpen(true);
    }
  };
  ...
  const handleModalReject = (id: string, reason: string) => {
    setReviewModalOpen(false);
      ...
      <ConfirmModal
        ...
        message={
          confirmModalState.action === 'approve' 
            ? "Are you sure you want to approve this course? It will be published immediately to the platform."
            : "Are you sure you want to reject this course? The instructor will be notified with the reason you provided."
        }
