...
import { 
  Megaphone, Search, Eye, EyeOff, Trash2, 
  Calendar, CheckCircle, Edit, Plus, AlertCircle, Clock
} from 'lucide-react';
...
const AdminAnnouncements = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setAnnouncements(announcements.map(a => 
          a.announcementId === selectedAnnouncement.announcementId ? { ...a, status: res.data.data.status } : a
        ));
      }
    } catch (error: any) {
  ...
  const handleTogglePublish = (announcement: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                currentItems.map((announcement) => (
                  <tr key={announcement.announcementId} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="py-4 px-6 align-top">
                      ...
                      {announcement.targetId && (
                        <span className="block text-xs text-body mt-1">ID: {announcement.targetId}</span>
                      )}
                    </td>
                    <td className="py-4 px-6 align-top">
        ...
        {/* Pagination Controls */}
        {totalPages > 1 && (
          <div className="p-4 border-t border-border flex items-center justify-center gap-2 bg-white rounded-b-xl">
            <button
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-body text-sm">
                {selectedAnnouncement.status === 'Published' 
                  ? `Are you sure you want to unpublish "${selectedAnnouncement.title}"? It will be removed from users' dashboards and sent back to Draft status.` 
                  : `Are you sure you want to publish "${selectedAnnouncement.title}"? It will immediately appear on dashboards for ${selectedAnnouncement.audience}.`}
              </p>
            </div>
            <div className="p-6 pt-0 flex items-center justify-end gap-3">
              <button 
              ...
              <button 
                ...
                className={`px-5 py-2.5 font-bold text-white rounded-xl shadow-sm transition-colors ${
                  selectedAnnouncement.status === 'Published' 
