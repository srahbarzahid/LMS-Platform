...
const AdminStudentDetails = () => {
  ...
  const handleDownloadInvoice = (payment: any) => {
    ...
    // Invoice Details
    doc.setFontSize(16);
    doc.setTextColor(31, 41, 55); // Heading color
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-8 bg-white p-6 rounded-3xl border border-border shadow-sm">
        ...
        <div className="ml-auto flex gap-3">
          ...
          <button onClick={() => { setEditFormData(student); setEditProfileModalOpen(true); }} className="px-4 py-2 bg-primary/10 text-primary font-bold rounded-xl hover:bg-primary/20 transition-colors">Edit Profile</button>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-4 mt-8">
              ...
              {enrolledCoursesList.map(course => (
                <div key={course.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 border border-border rounded-xl hover:bg-gray-50 transition-colors">
              ...
              {activities.map((activity) => {
                ...
                return (
                  <div key={activity.id} className="relative pl-8 md:pl-10">
                    {/* Timeline Dot */}
                    <div className={`absolute -left-[11px] top-1 w-5 h-5 rounded-full border-4 border-white ${activity.dot}`}></div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-2xl shadow-2xl mt-10 md:mt-0 max-h-[90vh] overflow-y-auto">
              ...
              <div className="md:col-span-1 flex flex-col items-center justify-center p-6 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100 shadow-sm">
                <div className="relative w-28 h-28 mb-4">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <circle cx="18" cy="18" r="16" fill="none" className="stroke-primary drop-shadow-md" strokeWidth="4" strokeDasharray="100" strokeDashoffset={100 - selectedCourseProgress.progress} strokeLinecap="round"></circle>
                  </svg>
              ...
              <div className="md:col-span-2 flex flex-col justify-center gap-4 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
                <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-bold shadow-sm ${
                    selectedCourseProgress.status === 'Completed' ? 'bg-emerald-100 text-emerald-700' :
                    selectedCourseProgress.status === 'In Progress' ? 'bg-blue-100 text-blue-700' :
                    'bg-gray-100 text-gray-700'
                  }`}>{selectedCourseProgress.status}</span>
                </div>
                <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-black text-primary">
                    {selectedCourseProgress.modules.filter((m:any) => m.status === 'Completed').length} <span className="text-caption font-medium">/ {selectedCourseProgress.modules.length}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {selectedCourseProgress.modules.map((mod: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl bg-gray-50/50 hover:bg-white hover:border-gray-200 hover:shadow-sm transition-all">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shadow-sm ${
                      ...
                      'bg-white text-caption border border-gray-200'
                    }`}>
                  ...
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm ${
                    mod.status === 'Completed' ? 'text-emerald-700 bg-emerald-100' :
                    mod.status === 'In Progress' ? 'text-blue-700 bg-blue-100' :
