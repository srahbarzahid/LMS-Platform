...
const AdminAdminDetails = () => {
  ...
  const [admin, setAdmin] = useState({
    ...
    permissions: [
      'Manage Users', 'Manage Courses', 'Manage Financials', 'System Settings', 'Platform Analytics'
    ]
  });
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-8 bg-white p-6 rounded-3xl border border-border shadow-sm">
        ...
        <div className="ml-auto flex gap-3">
          ...
          <button onClick={() => { setEditFormData(admin); setEditProfileModalOpen(true); }} className="px-4 py-2 bg-primary/10 text-primary font-bold rounded-xl hover:bg-primary/20 transition-colors">Edit Profile</button>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 max-w-2xl">
              {admin.permissions.map(perm => (
                <div key={perm} className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-border">
            ...
            {admin.role === 'Super Admin' && (
              <p className="mt-6 text-sm text-caption">Super Admins inherently possess all platform permissions.</p>
            )}
          </div>
        )}
