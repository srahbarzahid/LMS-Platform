...
import UserTable, { type Column } from '../../../components/admin/users/UserTable';
...
const AdminAdmins = () => {
  ...
  const handleAction = async (action: string, ids: string[], payload?: any) => {
    try {
      if (action === 'delete') {
        setConfirmDialog({
          isOpen: true,
  ...
  const columns: Column[] = [
    ...
    { 
      ...
      label: 'Role',
      render: (val) => (
        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${val === 'Super Admin' ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-700'}`}>
          {val}
        </span>
      )
    },
