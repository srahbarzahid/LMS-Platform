...
const AdminInstructorDetails = () => {
  ...
  const [selectedCoursePerformance, setSelectedCoursePerformance] = useState<any>(null);
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-6 mb-8 bg-white p-6 rounded-3xl border border-border shadow-sm">
        ...
        <div className="ml-auto flex gap-3">
          ...
          <button onClick={() => { setEditFormData(instructor); setEditProfileModalOpen(true); }} className="px-4 py-2 bg-primary/10 text-primary font-bold rounded-xl hover:bg-primary/20 transition-colors">Edit Profile</button>
        </div>
      </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="col-span-2">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {instructor.skills.split(', ').map(skill => (
                      <span key={skill} className="px-3 py-1 bg-gray-100 text-caption rounded-lg text-sm font-medium">{skill}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-2xl shadow-2xl mt-10 md:mt-0 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {selectedCoursePerformance.topReviews.map((review: any, idx: number) => (
                <div key={idx} className="p-4 border border-gray-100 rounded-xl bg-white hover:shadow-sm transition-all">
