...
const AdminCategories = () => {
  ...
  const columns = [
    ...
    },
    {
      key: "description",
      label: "Description",
      render: (val) => <div className="text-caption truncate max-w-[250px]" title={val}>
          {val || "-"}
        </div>
    },
    {
    ...
    },
    {
      key: "featured",
      label: "Featured",
      render: (val) => val ? <span className="px-2.5 py-1 rounded-full text-xs font-bold border bg-purple-50 text-purple-600 border-purple-200 inline-flex items-center gap-1">
            <Star className="w-3 h-3 fill-current" /> Yes
          </span> : <span className="px-2.5 py-1 rounded-full text-xs font-bold border bg-gray-100 text-gray-500 border-gray-200">
            No
          </span>
    },
    {
  ...
  />
    ...
    message={actionConfirm.type === "delete" ? `Are you sure you want to delete the "${actionConfirm.category?.name}" category? This action cannot be undone.` : `Are you sure you want to ${actionConfirm.category?.featured ? "unfeature" : "feature"} the "${actionConfirm.category?.name}" category?`}
...
var stdin_default = AdminCategories;
export {
