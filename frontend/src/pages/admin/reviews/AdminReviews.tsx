...
import { 
  ...
  MessageSquare, User, BookOpen, AlertTriangle,
...
const AdminReviews = () => {
  ...
  const handleHideUnhide = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        fetchData(); // Refresh data to update summary and list
      }
    } catch (error) {
  ...
  const filteredReviews = reviews.filter(r => {
    const matchesSearch = 
      ...
      (r.reviewDescription && r.reviewDescription.toLowerCase().includes(searchQuery.toLowerCase()));
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                currentReviews.map((review) => (
                  <tr key={review.reviewId} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="py-4 px-6 align-top max-w-xs">
                      <div className="space-y-1.5">
                        ...
                        {review.reviewTitle && (
                          <p className="font-bold text-sm text-heading line-clamp-1">{review.reviewTitle}</p>
                        )}
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-body text-sm">
                {selectedReview.status === 'Published' 
                  ? 'Are you sure you want to hide this review? It will be removed from the public course page but stored in the system.' 
                  : 'Are you sure you want to unhide this review? It will be visible to everyone on the course page again.'}
              </p>
            </div>
