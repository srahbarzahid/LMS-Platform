...
import { 
  Award, Search, Filter, Download, XCircle, 
  Eye, Calendar, GraduationCap, User, BookOpen, Check
} from 'lucide-react';
...
const AdminIssuedCertificates = () => {
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      setRevokeReason('');
      // Update local state to reflect change without re-fetching immediately (or we could just re-fetch)
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button 
                          ...
                          className="p-2 text-caption hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Download">
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-1/2 p-8 flex flex-col">
              <div className="flex justify-between items-start mb-6">
                ...
                <button onClick={() => { setSelectedCert(null); setShowRevokeConfirm(false); }} className="text-caption hover:text-heading">×</button>
              </div>
              ...
              <div className="space-y-6 flex-1">
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-gray-50">
                  <div>
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold ${
                      selectedCert.status === 'Valid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {selectedCert.status}
                    </span>
                  </div>
                ...
                {selectedCert.status === 'Revoked' && selectedCert.revokeReason && (
...
export default AdminIssuedCertificates;
