...
const AdminCertificateTemplates = () => {
  ...
  const confirmDelete = (id, isDefault) => {
    if (isDefault) {
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm text-heading divide-y divide-border">
              {isLoading ? <tr>
                ...
                </tr> : filteredTemplates.length === 0 ? <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-caption">No templates found.</td>
                </tr> : filteredTemplates.map((template) => <tr key={template.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${template.status === "Active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
                        {template.status}
                      </span>
                    </td>
  ...
  >
    ...
    className="p-2 text-caption hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors"
    title="Edit"
  >
      ...
      {isModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-[95vw] h-[95vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {modalStep === 1 ? <div className="p-8 max-w-2xl mx-auto w-full space-y-6 overflow-y-auto">
                  ...
                  <div>
                    ...
                    <textarea rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} disabled={modalMode === "view"} className="w-full px-3 py-2 border border-border rounded-lg focus:ring-1 focus:ring-primary outline-none disabled:bg-gray-100" placeholder="Optional description" />
                  </div>
                  <div>
  ...
  >
                  ...
                  {formData.background && <div className="mt-4 border border-border rounded-lg p-2 bg-gray-50">
                      <p className="text-xs text-caption mb-2 font-medium">Background Preview:</p>
                      {formData.background.startsWith("data:application/pdf") ? <div className="flex flex-col items-center justify-center py-8 text-gray-500 bg-white border border-gray-200 rounded">
                           <FileText className="w-12 h-12 mb-2 text-primary/50" />
                           <p className="text-sm font-medium">PDF File Uploaded</p>
                        </div> : <img src={formData.background} alt="Preview" className="w-full max-h-64 object-contain rounded" />}
                    </div>}
                </div> : <div className={`flex-1 overflow-hidden p-4 ${modalMode === "view" ? "pointer-events-none" : ""}`}>
  ...
  />
            ...
            <div className="p-5 border-t border-border bg-white flex justify-between items-center relative z-10 shadow-[0_-4px_10px_-4px_rgba(0,0,0,0.05)]">
              {modalStep === 2 ? <button type="button" onClick={() => setModalStep(1)} className="px-6 py-2.5 border border-border text-heading rounded-xl hover:bg-gray-50 transition-all font-medium flex items-center gap-2 hover:-translate-y-0.5">Back</button> : <div />}
              ...
              <div className="flex gap-3 items-center">
                ...
                {modalMode !== "view" && (modalStep === 1 ? <button
    type="button"
  ...
  >
                    ...
                    </button> : <button type="button" onClick={handleSave} className="px-8 py-2.5 bg-gradient-to-r from-orange-400 to-orange-500 hover:from-orange-500 hover:to-orange-600 text-white rounded-xl hover:shadow-lg hover:shadow-orange-500/30 hover:-translate-y-0.5 transition-all font-medium flex items-center gap-2">
...
var stdin_default = AdminCertificateTemplates;
export {
