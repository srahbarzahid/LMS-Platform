...
const Courses = () => {
    ...
    /* Course Grid */
          ...
          <div className="flex-grow">
            <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl border border-border shadow-sm">
              <span className="text-body font-medium">Showing <strong className="text-heading">{filteredCourses.length}</strong> of {courses.length} results</span>
              <CustomDropdown
                value={sortBy}
                onChange={setSortBy}
                options={sortOptions}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {loading ? <div className="col-span-full py-12 text-center text-body">Loading courses...</div> : filteredCourses.map((course, i) => <div key={course.id || i} className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-xl transition-shadow flex flex-col group cursor-pointer relative">
  ...
  >
                      ...
                      {course.featured && <span className="absolute top-3 left-3 bg-yellow-400 text-heading px-2 py-1 text-xs font-bold rounded flex items-center gap-1 shadow-sm">
