...
const demoCoursesList = [
  {
    ...
    subtitle: "Learn step-by-step with practical hands-on projects.",
    author: "PiTech Instructor 1",
    instructor: { name: "PiTech Instructor 1" },
    price: "1999",
    priceNum: 1999,
    rating: "4.9",
    reviews: "850",
  ...
  },
  {
    ...
    subtitle: "Master robot operating system and inverse kinematics simulation.",
    author: "PiTech Instructor 2",
    instructor: { name: "PiTech Instructor 2" },
    price: "2499",
    priceNum: 2499,
    rating: "4.8",
    reviews: "620",
    ...
    <div className="bg-[#f8f9fa] dark:bg-neutral-950 min-h-screen pt-32 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
          <div>
            <h1 className="text-4xl font-heading font-bold text-heading dark:text-white">All Courses</h1>
            <p className="text-body dark:text-neutral-400 mt-2 text-lg">Browse our extensive library of technology courses.</p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full lg:w-72 flex-shrink-0">
            <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-border dark:border-neutral-800 sticky top-24 shadow-sm">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {paginatedCourses.map((course, i) => (
                  <div key={course.id || i} className="bg-white dark:bg-neutral-900 rounded-2xl border border-border dark:border-neutral-800 overflow-hidden hover:shadow-xl transition-shadow flex flex-col group cursor-pointer relative">
                    <div className="relative h-48 overflow-hidden bg-gray-100 dark:bg-neutral-800">
                      <img src={course.img || `https://picsum.photos/seed/${course.id}/500/300`} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ...
                    <div className="p-6 flex-grow flex flex-col">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-bold text-heading dark:text-white">{course.rating || "4.5"}</span>
                        </div>
                      </div>
                      
                      <h3 className="font-heading font-bold text-heading dark:text-white text-lg mb-2 line-clamp-2 leading-tight group-hover:text-primary transition-colors">{course.title}</h3>
                      <p className="text-sm text-body dark:text-neutral-400 mb-4 line-clamp-2">{course.subtitle || "Learn everything you need to know with hands-on projects."}</p>
                      
                      <div className="flex items-center gap-2 mb-4">
                        <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(course.instructor?.name || course.author || "Instructor")}&background=random`} alt="Instructor" className="w-6 h-6 rounded-full" />
                        <span className="text-xs font-medium text-body dark:text-neutral-300">{course.instructor?.name || course.author || "Instructor"}</span>
                      </div>

                      <div className="mt-auto flex justify-between items-center pt-4 border-t border-border dark:border-neutral-800">
                        <div className="flex flex-col">
                          <span className="text-lg font-black text-heading dark:text-white">
                            {String(course.price).startsWith("₹") || course.price === "Free" ? course.price : `₹${course.price}`}
                          </span>
                        </div>
                        <Link to={`/courses/${course.id}`} className="px-5 py-2 bg-white dark:bg-neutral-800 border-2 border-primary text-primary font-bold rounded-xl hover:bg-primary hover:text-white transition-colors text-sm">
                          View Details
                        </Link>
                      </div>
                    </div>
