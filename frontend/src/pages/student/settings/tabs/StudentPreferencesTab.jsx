...
const StudentPreferencesTab = () => {
  ...
  const fetchPreferences = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get(normalizeApiPath("/student/settings/preferences"));
      const data = res.data;
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await apiClient.patch(normalizeApiPath("/student/settings/preferences"), {
        preferredLanguage,
        theme
      });
      const data = res.data;
  ...
  return (
    <form onSubmit={handleSave} className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-caption">{t("pref.subtitle", "Customize your student dashboard language and theme mode.")}</p>
      </div>

      {/* Language Selector */}
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-border p-6 space-y-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-caption">{t("pref.languageDesc", "Select your preferred language for learning navigation and buttons.")}</p>
          </div>
        </div>
        ...
        <div className="max-w-md pt-2">
          <select
            ...
            className="w-full px-4 py-3 bg-gray-50 dark:bg-neutral-800 border border-border rounded-xl text-sm font-medium text-heading focus:bg-white dark:focus:bg-neutral-900 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-border p-6 space-y-4 shadow-sm">
        <div>
          ...
          <p className="text-xs text-caption">{t("pref.themeDesc", "Select how the student portal looks to you.")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          ...
          <div
            ...
            className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex flex-col justify-between h-36 ${
              theme === "Light" ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:border-gray-300 dark:hover:border-neutral-700 bg-gray-50/50 dark:bg-neutral-800/50"
            }`}
          >
          ...
          <div
            ...
            className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex flex-col justify-between h-36 ${
              theme === "Dark" ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:border-gray-300 dark:hover:border-neutral-700 bg-gray-50/50 dark:bg-neutral-800/50"
            }`}
          >
            <div className="flex justify-between items-start">
              <div className="w-10 h-10 rounded-xl bg-gray-800 dark:bg-gray-700 flex items-center justify-center text-gray-200">
          ...
          <div
            ...
            className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex flex-col justify-between h-36 ${
              theme === "System" ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:border-gray-300 dark:hover:border-neutral-700 bg-gray-50/50 dark:bg-neutral-800/50"
            }`}
          >
            <div className="flex justify-between items-start">
