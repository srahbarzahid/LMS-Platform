...
const mockFrontendNotifications = [
  { id: 'n1', title: 'Assignment Graded', message: 'Your assignment "Build a Weather Station" has been graded. You received 95/100.', category: 'Assignments', isRead: false, createdAt: new Date(Date.now() - 1000 * 60 * 60 * 2), relatedUrl: '/student/assignments' },
  { id: 'n2', title: 'New Course Available', message: 'A new course "Advanced Robotics" has just been published. Enroll now!', category: 'Course Updates', isRead: true, createdAt: new Date(Date.now() - 1000 * 60 * 60 * 48), relatedUrl: '/student/my-courses' },
  { id: 'n3', title: 'Certificate Earned', message: 'Congratulations! You have earned a certificate for IoT Fundamentals.', category: 'Certificates', isRead: false, createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 5), relatedUrl: '/student/certificates' },
];

const getCategoryIcon = (category: string) => {
  switch (category) {
    case 'Course Updates': return <BookOpen className="w-5 h-5" />;
    case 'Assignments': return <ClipboardList className="w-5 h-5" />;
    case 'Quizzes': return <CheckSquare className="w-5 h-5" />;
    ...
    case 'Platform Announcements': return <Megaphone className="w-5 h-5" />;
...
const getCategoryColor = (category: string) => {
  switch (category) {
    case 'Assignments': return 'bg-orange-100 text-orange-600';
    case 'Course Updates': return 'bg-blue-100 text-blue-600';
    case 'Certificates': return 'bg-emerald-100 text-emerald-600';
...
const StudentNotifications = () => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/student/announcements', { withCredentials: true });
        if (res.data.success && res.data.data.length > 0) {
          const announcementNotifications = res.data.data.map((ann: any) => ({
          ...
          setNotifications(prev => {
            // Remove any existing platform announcements to prevent duplicates if re-fetched
  ...
  return (
    <div className="space-y-8 pb-8 max-w-7xl">
      ...
      <div className="space-y-4">
        {filteredNotifications.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 border border-border border-dashed text-center shadow-sm">
