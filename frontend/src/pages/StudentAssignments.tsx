...
const mockAssignments = [
  {
    ...
    maxMarks: 100,
    status: 'Pending', // Pending, Submitted, Under Review, Graded, Overdue
    instructor: 'Hisham',
  },
  {
    id: 2,
...
const StudentAssignments = () => {
  ...
  const handleAction = (assignmentId: number) => {
      ...
      {filteredAssignments.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredAssignments.map(assignment => (
            <div key={assignment.id} className="bg-white rounded-3xl border border-border shadow-sm p-6 flex flex-col hover:shadow-lg transition-shadow">
        ...
        <div className="bg-white rounded-3xl border border-border p-12 text-center shadow-sm flex flex-col items-center justify-center">
          ...
          <p className="text-body max-w-md mx-auto mb-8">
            {searchQuery 
              ...
              : 'Enroll in courses that offer practical assignments to build your portfolio.'}
          </p>
          {!searchQuery && (
